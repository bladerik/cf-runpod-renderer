...
ARG DEBIAN_FRONTEND=noninteractive
RUN apt-get update && apt-get install -y software-properties-common && add-apt-repository -y ppa:graphics-drivers/ppa && apt-get update && apt-get install -y vulkan-tools && apt-get clean
RUN apt-get update && apt-get install -y vulkan-tools && libnvidia-gl-525 && apt-get clean
